...
const actionAnalyzePersonalityToSuccessTestAgent = 'action_analyze_personality_to_success_test_agent';
const rrel1 = 'rrel_1';

const baseKeynodes = [
  { id: question, type: ScType.NodeConstClass },
  { id: actionAnalyzePersonalityToSuccessTestAgent, type: ScType.NodeConstClass },
...
const describeAgent = async (
  ...
  template.triple(
      keynodes[actionAnalyzePersonalityToSuccessTestAgent],
      ScType.EdgeAccessVarPosPerm,
      actionNodeAlias
  );
...
export const analyzePersonalityToSuccessTestAgent = async (userAnswers: object) => {
