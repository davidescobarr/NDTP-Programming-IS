...
const actionAnalyzeNeedInApprovalTestAgent = 'action_analyze_need_in_approval_test_agent';
const rrel1 = 'rrel_1';

const baseKeynodes = [
  { id: question, type: ScType.NodeConstClass },
  { id: actionAnalyzeNeedInApprovalTestAgent, type: ScType.NodeConstClass },
...
const describeAgent = async (
  ...
  template.triple(
      keynodes[actionAnalyzeNeedInApprovalTestAgent],
      ScType.EdgeAccessVarPosPerm,
      actionNodeAlias
  );
...
export const analyzeNeedInApprovalTestAgent = async (userAnswers: object) => {
