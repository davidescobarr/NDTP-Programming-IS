...
const actionAnalyzeIovaishiTestAgent = 'action_analyze_iovaishi_test_agent';
const rrel1 = 'rrel_1';

// Обновлённый базовый набор ключевых узлов — обязательно типы должны быть корректными
const baseKeynodes = [
  { id: question, type: ScType.NodeConstClass },
  { id: actionAnalyzeIovaishiTestAgent, type: ScType.NodeConstClass },
...
const describeAgent = async (
  ...
  template.triple(
      keynodes[actionAnalyzeIovaishiTestAgent],
      ScType.EdgeAccessVarPosPerm,
      actionNodeAlias
  );
  // 3. Передаём JSON‑ссылку как параметр через tripleWithRelation с ролью rrel2
...
export const analyzeIovaishiTestAgent = async (userAnswers: object) => {
