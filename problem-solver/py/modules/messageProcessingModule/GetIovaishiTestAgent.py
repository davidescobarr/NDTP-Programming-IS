...
class GetIovaishiTestAgent(ScAgentClassic):
    def __init__(self):
        super().__init__("action_get_iovaishi_test_agent")

    def on_event(self, event_element: ScAddr, event_edge: ScAddr, action_element: ScAddr) -> ScResult:
        result = self.run(action_element)
        is_successful = result == ScResult.OK
        finish_action_with_status(action_element, is_successful)
        self.logger.info("GetIovaishiTestAgent finished %s",
                         "successfully" if is_successful else "unsuccessfully")
        return result

    def run(self, action_node: ScAddr) -> ScResult:
        self.logger.info("GetIovaishiTestAgent started")
        ...
        for item in search_result:
            ...
            for name_item in name_result:
                ...
                if test_name == "Тест Йовайши":
                    self.logger.info("Found Iovaishi Test: %s", test_name)
        ...
        self.logger.warning("Iovaishi Test not found")
        return ScResult.ERROR_NOT_FOUND
