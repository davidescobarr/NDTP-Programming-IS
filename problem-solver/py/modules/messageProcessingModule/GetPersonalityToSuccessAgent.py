...
class GetPersonalityToSuccessAgent(ScAgentClassic):
    def __init__(self):
        super().__init__("action_get_personality_to_success_test_agent")

    def on_event(self, event_element: ScAddr, event_edge: ScAddr, action_element: ScAddr) -> ScResult:
        result = self.run(action_element)
        is_successful = result == ScResult.OK
        finish_action_with_status(action_element, is_successful)
        self.logger.info("GetPersonalityToSuccessAgent finished %s",
                         "successfully" if is_successful else "unsuccessfully")
        return result

    def run(self, action_node: ScAddr) -> ScResult:
        self.logger.info("GetPersonalityToSuccessAgent started")
        ...
        for item in search_result:
            ...
            for name_item in name_result:
                ...
                if test_name == "Методика диагностики личности на мотивацию к успеху Т. Элерса":
                    self.logger.info("Found GetPersonalityToSuccess Test: %s", test_name)
                    ...
                    if not json_result:
                        self.logger.warning("No JSON questions link found for Motivational Test")
                        return ScResult.ERROR_NOT_FOUND
                    ...
                    for json_item in json_result:
                        ...
                        if json_content:
                            ...
                            self.logger.info("PersonalityToSuccess Test JSON returned successfully")
                            return ScResult.OK

        self.logger.warning("PersonalityToSuccess Test not found")
        return ScResult.ERROR_NOT_FOUND
