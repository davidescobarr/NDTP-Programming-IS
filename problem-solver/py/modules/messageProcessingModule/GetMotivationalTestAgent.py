...
class GetMotivationalTestAgent(ScAgentClassic):
    def __init__(self):
        super().__init__("action_get_motivational_test_agent")

    def on_event(self, event_element: ScAddr, event_edge: ScAddr, action_element: ScAddr) -> ScResult:
        result = self.run(action_element)
        is_successful = result == ScResult.OK
        finish_action_with_status(action_element, is_successful)
        self.logger.info("GetMotivationalTestAgent finished %s",
                         "successfully" if is_successful else "unsuccessfully")
        return result

    def run(self, action_node: ScAddr) -> ScResult:
        self.logger.info("GetMotivationalTestAgent started")
        ...
        for item in search_result:
            ...
            for name_item in name_result:
                ...
                if test_name == "Мотивы выбора профессии":
                    self.logger.info("Found Motivational Test: %s", test_name)
                    ...
                    for json_item in json_result:
                        ...
                        if json_content:
                            ...
                            self.logger.info("Motivational Test JSON returned successfully")
                            return ScResult.OK

        self.logger.warning("Motivational Test not found")
        return ScResult.ERROR_NOT_FOUND
