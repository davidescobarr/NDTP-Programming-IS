...
class GetTechinicianLevelTestAgent(ScAgentClassic):
    def __init__(self):
        super().__init__("action_get_technician_level_test_agent")

    def on_event(self, event_element: ScAddr, event_edge: ScAddr, action_element: ScAddr) -> ScResult:
        result = self.run(action_element)
        is_successful = result == ScResult.OK
        finish_action_with_status(action_element, is_successful)
        self.logger.info("GetTechinicianLevelTestAgent finished %s",
                         "successfully" if is_successful else "unsuccessfully")
        return result

    def run(self, action_node: ScAddr) -> ScResult:
        self.logger.info("GetTechinicianLevelTestAgent started")
        ...
        for item in search_result:
            ...
            for name_item in name_result:
                ...
                if test_name == "Оценка склонности к техническим наукам":
                    self.logger.info("Found TechnicianLevel Test: %s", test_name)
                    ...
                    if not json_result:
                        self.logger.warning("No JSON questions link found for TechnicianLevel Test")
                        return ScResult.ERROR_NOT_FOUND
                    ...
                    for json_item in json_result:
                        ...
                        if json_content:
                            ...
                            self.logger.info("TechnicianLevel Test JSON returned successfully")
                            return ScResult.OK

        self.logger.warning("TechnicianLevel Test not found")
        return ScResult.ERROR_NOT_FOUND
